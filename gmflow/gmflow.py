...
class GMFlow(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        for scale_idx in range(self.num_scales): #~num_scales=1 只循环一次
            ...
            #~ 保存transformer 的输出来送到fpp的分支去，把两个特征并起来用，skip_feature 也是并起来的 在Batch维度并起来的 2025年9月9日
            ...
            #~ ↓上采样后加入到结果列表 为了效率使用最简单的双线性插值 1 上采样 2025年9月5日 下面这个双线性插值后的结果一定保存到flow_preds中 2025年9月11日
            ...
            #~ 2 bilinear 上采样 2025年9月5日 由于循环只执行一次if条件为false，2这一步没有执行 2025年9月11日
        ...
        results_dict.update({'flow_preds': flow_preds,'depth1_pred': depth10, 'depth2_pred':depth11})
